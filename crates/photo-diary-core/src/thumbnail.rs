...
/// Saves a thumbnail of an already-decoded (and orientation-corrected) image to `dst`,
/// with its long edge within `max_edge`.
/// - Aspect ratio is preserved. Does not upscale if the source long edge is <= `max_edge`.
/// - Output format is determined by `dst`'s extension (webp recommended).
/// - Returns the (width, height) of the generated thumbnail.
///
/// Contract: takes a decoded `&DynamicImage` (shared with AVIF encoding so the source is
/// decoded and oriented once) rather than a path.
pub fn make_thumbnail(img: &DynamicImage, dst: &Path, max_edge: u32) -> Result<(u32, u32)> {
...
mod tests {
    ...
    fn shrinks_landscape_keeping_aspect_and_writes_dst() {
        ...
        let (tw, th) = make_thumbnail(&img, &dst, 50).unwrap();
